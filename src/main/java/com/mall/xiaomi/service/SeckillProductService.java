...
//        }
        ...
        redisTemplate.delete(RedisKey.SECKILL_PRODUCT_LIST + seckillProduct.getTimeId());
    }
    ...
    public void deleteTime(Integer timeId) {
        seckillTimeMapper.deleteByPrimaryKey(timeId);
        Example example = new Example(SeckillProduct.class);
        example.createCriteria().andEqualTo("timeId", timeId);
        seckillProductMapper.deleteByExample(example);
    }

    public void addTime(SeckillTime seckillTime) {
        seckillTimeMapper.insert(seckillTime);
    }
    ...
    public void deleteSeckill(String seckillId) {
        try {
            SeckillProductVo seckill = seckillProductMapper.getSeckill(seckillId);
            redisTemplate.delete(RedisKey.SECKILL_PRODUCT_LIST + seckill.getTimeId());
            seckillProductMapper.deleteByPrimaryKey(seckillId);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
